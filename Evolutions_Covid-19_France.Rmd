...
stockdep %>% filter(name=='rea' & dep %in% c('14','36','67','75','93')) %>%
...
stockdep %>% left_join(lits, by=c('dep'='code_dept')) %>%
             ...
             filter(name=='rea' & dep %in% c('14','36','67','75','93')) %>%
...
nouveauxdep %>% filter(name=='décès' & dep %in% c('14','36','67','75','93')) %>%
                ggplot(aes(x = jour, y = value, color=dep)) + 
                  geom_line(aes(y = smoute(value))) + #rollmean(value, 7, fill=rep("extend",3)))) +
                  theme_minimal() +
                  scale_x_date(date_labels = "%d %b", date_breaks = "2 days") +
...
nouveauxdep %>% filter(name=='réanimation' & dep %in% c('14','36','67','75','93')) %>%
                ggplot(aes(x = jour, y = value, color=dep)) + 
